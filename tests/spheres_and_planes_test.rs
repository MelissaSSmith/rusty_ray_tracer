...
fn sphere_scene_test() {
    ...
    middle.set_transform(Matrix::translate(-0.5, 1.0, 0.5) * Matrix::rotate_x(PI/2.0));
    ...
    let backdrop_transform = Matrix::rotate_x(PI/2.0) * Matrix::translate(0.0, 0.0, 4.0);
    ...
    let objects: Vec<Box<dyn ShapeTrait>> = vec![Box::new(floor), Box::new(backdrop),
