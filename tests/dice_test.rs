...
fn dice_test() {
    ...
    for i in -2..=2 {
        for j in -4..=4 {
            ...
            let hue = rnd.random_range((0.0..1.0));
            ...
            let material2 = Material::create()
                .with_color(Color::create(hue, 0.8, 1.0))
                .with_diffuse(1.0);
            let size = rnd.random_range((0.05..0.1));
            let pos_x = i as f64 * 0.4 + rnd.random_range((-0.1..0.1));
            let pos_y = j as f64 * 0.4 + rnd.random_range((-0.1..0.1));

            let radians = rnd.random_range((0.0..2.0 * PI));
