...
#[test]
#[ignore]
fn test_multi_color_glamour_shot() {
    ...
    let ring_pattern = RingPattern::create(vec![WHITE, Color::create(0.5, 1.0, 0.5), Color::create(0.5, 0.5, 1.0), Color::create(1.0, 0.5, 0.5)])
        .with_transform(Matrix::translate(-1.5, -0.5, 0.5) * Matrix::scale(0.2, 0.2, 0.2));

    let sphere_2 = Shape::Sphere.create()
        .with_material(
            Material::create()
                .with_pattern(ring_pattern)
                .with_ambient(0.1)
                .with_specular(0.0)
                .with_diffuse(0.6)
                .with_reflective(0.3)
